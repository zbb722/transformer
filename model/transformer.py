...
class Transformer(nn.Module):
    ...
    def beam_search_decode(self, src, max_len=100, beam_size=5, sos_id=1, eos_id=2):
        """
        Beam Search 解码函数
        参数：
            src: (1, src_len) 源语言句子（只支持 batch_size = 1）
            max_len: 解码的最大长度
            beam_size: beam 宽度
            sos_id: 起始 token id
            eos_id: 终止 token id
        返回：
            最佳解码序列的 token id 列表
        """
        assert src.size(0) == 1, "只支持 batch_size=1 的解码"

        device = src.device
        memory = self.encode(src)  # 编码器输出

        # beam中每个元素是 (tokens, log_prob)
        sequences = [[torch.tensor([sos_id], dtype=torch.long, device=device), 0.0]]
        ...
        for _ in range(max_len):
            ...
            for seq, score in sequences:
                ...
                logits = self.decode(memory, tgt)  # (1, seq_len, vocab_size)
                ...
                for i in range(beam_size):
                    ...
                    candidate = (torch.cat([seq, next_token]), next_score)
                    all_candidates.append(candidate)

            # 选择得分最高的 beam_size 个序列
            ordered = sorted(all_candidates, key=lambda tup: tup[1], reverse=True)
            sequences = ordered[:beam_size]

            # 如果所有序列都以 <eos> 结尾，提前结束
            if all(seq[-1].item() == eos_id for seq, _ in sequences):
                break

        # 返回分数最高的序列（去掉起始标记）
        best_seq = sequences[0][0]
        return best_seq[1:].tolist()
